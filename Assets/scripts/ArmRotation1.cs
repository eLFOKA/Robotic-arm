...
public class ArmRotation1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Utility.using_axis == axis_index)
        {
            ...
            if (bufor >= rotation_min && bufor <= rotation_max) 
            {
                rotation_counter = bufor;
                arm.transform.RotateAround(axis_point.transform.position, new Vector3(0, 1, 0), (float)(rotationSpeed * read * Time.deltaTime));
            }
        }
    }
}
